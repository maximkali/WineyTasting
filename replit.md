...
Changelog:
- June 29, 2025 (PM). Major architectural change: Implemented unified setup flow (setup-unified.tsx) combining Home > Setup > Wine List > Rounds into a single continuous page. This eliminates navigation persistence issues by keeping all data in memory until final save. Added roundIndex field to bottle schema to support round assignments during bottle creation. Modified PUT /api/games/:gameId/bottles endpoint to accept roundIndex with each bottle. All setup steps now flow seamlessly without page refreshes, solving previous data loss issues during navigation.
