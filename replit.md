...
Changelog:
- June 29, 2025 (PM). Restored original setup, wine list (organize) and rounds pages to their untouched state while keeping the new professional home page design. Home page now creates game when user enters name, then navigates to setup page with gameId parameter - restoring the original flow. Fixed navigation to match original: Home (create game) → Setup → Wine List → Rounds.
- June 29, 2025 (PM). Restored original setup page UI and removed all first/last name collection from codebase per user request. Setup page now features beautiful 3-step wizard flow: Host name entry → Game configuration (players/bottles/rounds) → Review and create. Game ID is created when the final setup form is completed. User loved the original setup page UI and language, so it was completely preserved while removing name collection entirely.
- June 29, 2025 (PM). Complete professional redesign of home page per user feedback. Removed the middle card/box that user disliked and created modern landing page with hero section, compelling copy, prominent action buttons, and feature highlights. Kept logo at top and emoji navigation at bottom as requested. New design is more professional, consistent, and engaging with better typography and spacing.
- June 29, 2025 (PM). Simplified home page and setup flow per user request. Removed name fields from home page - now users just click "Let's Get Started!" to go to setup page where they'll enter host name. Updated button styling to muted pink (#C17A8C) and text styling to match provided design. Implemented warning dialog for "Back to Wine List" button that clears all wine data when user confirms. Added DELETE endpoint for bottles to support clean data clearing. Game creation now happens on setup page instead of home page.
